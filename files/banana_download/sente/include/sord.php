...
function getForm(){
	$q=mysql_query("select id from sord order by id desc limit 1");//used when formatting our reference number. //get only id of last record (order by id desc limit 1)
	...
	<form method="post" enctype="multipart/form-data" name="entry_form">
    	<div id="cust_name_"><input type="text" name="cust_name" placeholder="Customer Name" id="cust_name" /></div>
        <div id="teller_name">You were served by: <?php echo $_SESSION['logged']; ?></div>
    	<div id="form_title">
        	Sales Order <span id="ref">(SO - <?php echo $ref; ?>)</span>
        </div>
        <div id="o_date">
        ...
        <div id="form_comm">
        <textarea name="comm" placeholder="Comments, Terms, Conditions" id="comm">Thank you for shopping with us</textarea>
        </div>
    </form>
...
function saveEntry(){
	$teller=$_SESSION['logged'];
	$ref=$_POST['ref'];
	$supp=$_POST['supp'];
	if($supp==""){
		$supp="Random Customer";
	}
	$sbt=$_POST['sbt'];
	$tax=$_POST['tax'];
	$tot=$_POST['tot'];
	...
	$q=mysql_query("insert into sord (ref, cus, sbt, tax, tot, term, sta, teller) values ('$ref', '$supp', '$sbt', '$tax', '$tot', '$term', '$po_sta', '$teller')");
	if($q){
		...
		foreach($itmtbl as $rec){
			...
			$q=mysql_query("insert into soitm (itm, des, sord, sbt, vat, upr, qty) values ('$itm', '$des', @pord, '$sbt', '$vat', '$upr', '$qty')");
			if($q){//////////////////////////////////////ACCOUNTS AND INVENTORY ARE AFFECTED AT THIS POINT///////////////////////////////////////////////////////
				...
				$r=mysql_fetch_assoc($q);
				$p_itm=$r['id'];
				$q=mysql_query("insert into prole (par, vt, vr, pro, cr, sord) values ('$supp', 'Sales', '$inv', $p_itm, ($sbt+$vat), @pord)");////debit item account
...
function fetchEntry(){
	$q=mysql_query("select p.id, p.ref, p.cus, p.sbt, p.tot, p.sta, date_format(p.date, '%a, %M %d, %Y') as date
					from sord as p order by p.id desc");
	...
	if($q){
		...
		while($r=mysql_fetch_assoc($q)){
			$i++;
			$id=$r['id'];
			$q2=mysql_query("select date_format(date, '%a, %M %d, %Y') as prev from sord where id=(select max(id) from sord where id<$id)");
...
function moreInfo($infoId){
	...
	$q=mysql_query("select * from soitm where sord=$infoId");
	if(mysql_num_rows($q)<1){//if order is empty
		$q=mysql_query("select o.id as oid, o.ref, o.cus, o.sbt as osbt, o.tax, o.tot, o.sta, o.term, o.teller, date_format(o.date, '%a, %b %d, %Y') as date
							from sord as o where o.id=$infoId");
		if($q){
			$json_data='{';
			$i=0;
			while($r=mysql_fetch_assoc($q)){
				$i++;
				$oid=$r['oid']; $ref=$r['ref']; $sname=ucwords($r['cus']); $teller=ucwords($r['teller']); $osbt=$r['osbt']; $tax=$r['tax']; $tot=$r['tot']; $sta=$r['sta']; $term=$r['term']; $date=$r['date'];
				if($sname==""){
					$sname="Random Customer";
				}
				$json_data.='"rec'.$i.'":{"oid":"'.$oid.'", "ref":"'.$ref.'", "sname":"'.$sname.'", "osbt":"'.$osbt.'", "tax":"'.$tax.'", "tot":"'.$tot.'",
					"sta":"'.$sta.'", "term":"'.$term.'", "teller":"'.$teller.'", "level":"'.$level.'", "date":"'.$date.'"},';
			}
			$json_data=chop($json_data, ",");
			$json_data.='}';
			echo $json_data;
		...
		$q=mysql_query("select o.id as oid, o.ref, o.cus, o.sbt as osbt, o.tax, o.tot, o.sta, o.term, o.teller, date_format(o.date, '%a, %b %d, %Y') as date,
					i.itm, i.des, i.sbt as isbt, i.vat, i.upr, i.qty from sord as o join soitm as i
					on i.sord=o.id where o.id=$infoId");
		if($q){
			$json_data='{';
			$i=0;
			while($r=mysql_fetch_assoc($q)){
				...
				$json_data.='"rec'.$i.'":{"oid":"'.$oid.'", "ref":"'.$ref.'", "sname":"'.$sname.'", "osbt":"'.$osbt.'", "tax":"'.$tax.'", "tot":"'.$tot.'",
					"sta":"'.$sta.'", "term":"'.$term.'", "teller":"'.$teller.'", "level":"'.$level.'", "date":"'.$date.'","iid":"'.$iid.'","itm":"'.$itm.'","des":"'.$des.'","isbt":"'.$isbt.'",
...
function editEntry($infoId){
	$q=mysql_query("select * from soitm where sord=$infoId");
	if(mysql_num_rows($q)<1){
		$q1=mysql_query("select o.id as oid, o.ref, o.cus, o.sbt as osbt, o.tax, o.tot, o.sta, o.term, date_format(o.date, '%a, %b %d, %Y') as date from sord as o where o.id=$infoId");
	}
	else{
		$q1=mysql_query("select o.id as oid, o.ref, o.cus, o.sbt as osbt, o.tax, o.tot, o.sta, o.term, date_format(o.date, '%a, %b %d, %Y') as date,
					i.id as iid, i.itm, i.des, i.sbt as isbt, i.vat, i.upr, i.qty from sord as o join soitm as i
					on i.sord=o.id where o.id=$infoId");
	}
    ...
    <div id="form_wrapper">
	<form method="post" enctype="multipart/form-data" name="entry_form">
    	<div><input type="text" name="cust_name" placeholder="Customer Name" id="cust_name" value="<?php echo $r1['cus']; ?>" /></div>
    	<div id="form_title">
        	Sales Order <span id="ref"><?php echo $r1['ref']; ?></span>
        </div>
        <div id="o_date">
			<?php echo $r1['date']; ?>
        </div>
...
function upEntry($infoId){
	...
	print_r($itmtbl);
	//don't forget to insert into status
	$q=mysql_query("update sord set ref='$ref', cus='$supp', sbt='$sbt', tax='$tax', tot='$tot', term='$term' where id=$infoId");
	if($q){
		echo "THANK YOU JESUS<br />";
		/*/
			first, capture the quantities sold previously for all the items in the order - from "soitm"
			add the qty to the respective itm in "product" to signify a return (we may do a debit here, or just increase inventory)
			then, delete from "soitm" all the items with that order number
			plus, delete from "prole" all that pertains to that order number (we need a sord field for prole) (but we need to indicate a return - how?)
			then we can start doing the updates
			normally update the record. soitm, prole, reduce qty in product
			...
			to get how much was returned, we increase inventory but reduce sales. we increase ret inwards but red debtors () and cash

		/*/
		$q=mysql_query("select itm, qty from soitm where sord=$infoId");
		while($r=mysql_fetch_assoc($q)){
			$sold_itm=$r['itm'];
		...
		$q=mysql_query("delete from soitm where sord=$infoId");
		$q=mysql_query("delete from prole where sord=$infoId and vt='Sales'");
		//loop through the items array
		foreach($itmtbl as $rec){
			if(sizeof($rec)>1){
				//get each record values
				...
				$q=mysql_query("insert into soitm (itm, des, sord, sbt, vat, upr, qty) values ('$itm', '$des', $infoId, '$sbt', '$vat', '$upr', '$qty')");
				if($q){//////////////////////////////////////ACCOUNTS AND INVENTORY ARE AFFECTED AT THIS POINT///////////////////////////////////////////////////////
					$inv=$ref;
					$inv=end(explode("-", $inv));//get the las part of the order reference
					$inv="SR - ".$inv;
					...
					$q=mysql_query("insert into prole (par, vt, vr, pro, cr, sord) values ('$supp', 'Sales', '$inv', $p_itm, ($sbt+$vat), $infoId)");//debit item account
...
function delEntry($infoId){
	...
	if($q){
		//now delete the PO
		$q=mysql_query("delete from sord where id=$infoId");
	}
}

function itmForm(){
	$q=mysql_query("select i.id as id, i.name as name, i.price as price from product as i order by i.name asc");
	?>
	<div id="itm_form">
		<div id="itm_inner">
			...
			<div id="itm_tsk">
				...
				/*
				while($r=mysql_fetch_assoc($q)){
					echo "<option value='".$r['id']."' title='".$r['price']."' label='".$r['name']."'>".$r['name']."</option>";//unit price is returned as the option title
				}
				*/
				?>
				</select>
                -->
                <input type="text" placeholder="Search for an Item" id="itm" /><!--inline event listeners are suicidal - don't use them please! use javascript-->
...
function getOitm($infoId){
	$q=mysql_query("select o.id as oid, o.ref, o.cus, o.sbt as osbt, o.tax, o.tot, o.sta, o.term, date_format(o.date, '%a, %b %d, %Y') as date,
					i.id as iid, i.itm, i.des, i.sbt as isbt, i.vat, i.upr, i.qty from sord as o join soitm as i on i.sord=o.id where o.id=$infoId");
	$json_data='{';
	$i=0;
	while($r=mysql_fetch_assoc($q)){
		$i++;
		$oid=$r['oid']; $ref=$r['ref']; $sname=$r['cus']; $osbt=$r['osbt']; $tax=$r['tax']; $tot=$r['tot']; $sta=$r['sta']; $term=$r['term'];
...
function appro($infoId){
	...
	$q=mysql_query("select ref from sord where id=$infoId");
	$r=mysql_fetch_assoc($q);
	$ref=$r['ref'];
	/////
	$q=mysql_query("select itm, qty from soitm where sord=$infoId");
