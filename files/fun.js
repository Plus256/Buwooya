...
buffer.render=function(){
	...
	var statements=new Array("Fetching Content", "Banana Village", "Holiday and Retreat Centre");
	var thisStatement=0;
...
function activeMenuItem(index){
	...
	//then set current
	menu_items[index].style.background="#060";
	menu_items[index].style.color="#FC0";
}

function getRev(){
	var xhr;
	var url="json/rev.json";
	if(window.XMLHttpRequest){
		xhr=new XMLHttpRequest();
	}
	else{
	...
	xhr.onreadystatechange=function(){
		...
		if(xhr.readyState==4 && xhr.status==200){
			var data=xhr.responseText;//u don't have to define data as an array like so, var data=new Array - both approaches work, however
				data=JSON.parse(data);//be sure to clear cache after changing JSON file
			for(var i=0; i<3; i++){
				var guest_review=_("div");
					guest_review.setAttribute("id", "guest_review");
				//////////////////////////////////////////////////////////////////
				var guest_review_quote=_("div");
					guest_review_quote.innerHTML="&quot;";
				var guest_review_words=_("div");
					guest_review_words.setAttribute("id", "guest_review_words");
					guest_review_words.innerHTML=data[i].words;
				/////////////////////////////////////////////////////////////////
				var guest_review_guest=_("div");
					guest_review_guest.setAttribute("id", "guest_review_guest");
					guest_review_guest.innerHTML=data[i].guest;
				////////////////////////////////////////////////////////////////
				var guest_review_from=_("div");
					guest_review_from.setAttribute("id", "guest_review_from");
					guest_review_from.innerHTML=data[i].from;
				///////////////////////////////////////////////////////////////
				guest_review.appendChild(guest_review_words);
				guest_review.appendChild(guest_review_guest);
				guest_review.appendChild(guest_review_from);
				$("reviews_body").appendChild(guest_review);
			}
		}
	}
	xhr.send(null);
...
function DialogBox(){
	...
	this.render=function(){
		var viewport_size=window.matchMedia('@media only screen and (max-width:640px)');
		var win_width=window.innerWidth || document.body.clientWidth;
		var win_height=window.innerHeight || document.body.clientHeight;
